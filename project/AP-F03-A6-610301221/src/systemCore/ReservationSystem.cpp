...
void ReservationSystem::initialize(const string &restaurantsFile,
  ...
  try {
    ...
    restaurants_ = dataLoader_->loadRestaurants(restaurantsFile, districtMap_);
...
Response ReservationSystem::reserve(const string &restaurantName, int tableId,
  ...
  int totalPrice = 0;
  if (!foods.empty()) {
    auto [foodsValid, price] = validateAndCalculateOrder(restaurant, foods);
    if (!foodsValid) {
      return NotFoundResponse();
    }
    totalPrice = price;
  ...
  string responseOut =
      formatReservationResponse(reservationId, restaurantName, tableId,
                                startTime, endTime, foods, totalPrice);
...
Response ReservationSystem::deleteReserve(const string &restaurantName,
  ...
  auto it = find_if(userReservations.begin(), userReservations.end(),
  ...
  Table *table = restaurant->findTable(it->getTableId());
  if (!table) {
    return NotFoundResponse();
  }
