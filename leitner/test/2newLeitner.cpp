...
void App::pickCards(int &number) {
  ...
  for (int type = MONTHLY; type > DAILY; --type) {
    if (currentDay % interval[type] == 0 && cardCounter < number) {
      list<FlashCard *> cardsToMove;
      ...
      for (FlashCard *card : cardsToMove) {
        moveCard(card, boxes[type], boxes[REVIEW_BOX]);
      }
    }
  }
  ...
  if (cardCounter < number) {
    list<FlashCard *> cardsToMove;
    for (FlashCard *card : boxes[DAILY].cards) {
      if (cardCounter < number) {
        cardsToMove.push_back(card);
        cardCounter++;
      }
    ...
    for (auto card : cardsToMove) {
      moveCard(card, boxes[DAILY], boxes[REVIEW_BOX]);
    }
  }
}

void App::reviewCards() {
  list<FlashCard *> cardsToReview = boxes[REVIEW_BOX].cards;
  activeDays++;
  ...
  for (FlashCard *card : cardsToReview) {
    ...
    if (ansValue) {
      ...
      if (card->boxNum < SAVED_IN_MEMORY) {
        int newBoxNum = min(card->boxNum + 1, SAVED_IN_MEMORY);
        moveCard(card, boxes[REVIEW_BOX], boxes[newBoxNum]);
      }
    } else {
      ...
      if (card->wrongAns > 1 && (card->boxNum > DAILY) &&
          (card->boxNum != SAVED_IN_MEMORY)) {
        int newBoxNum = card->boxNum - 1;
        moveCard(card, boxes[REVIEW_BOX], boxes[newBoxNum]);
      }
    }
  }
