...
Response PostIncreaseBudgetCmd::execute(ReservationSystem &system) {
  if (!system.isLoggedIn()) {
    return PermissionDeniedResponse();
  }
  ...
  auto [budgetParams, _] = parseParameters(params_);

  if (budgetParams.find(AMOUNT) == budgetParams.end() ||
      budgetParams[AMOUNT].empty()) {
    return BadRequestResponse();
  }

  int amount;
  try {
    amount = std::stoi(budgetParams[AMOUNT]);
  } catch (...) {
    return BadRequestResponse();
  }

  return amount > 0 ? system.increaseBudget(amount) : BadRequestResponse();
}
...
Response GetShowBudgetCmd::execute(ReservationSystem &system) {
  if (!system.isLoggedIn()) {
    return PermissionDeniedResponse();
  }

  return system.showBudget();
}
